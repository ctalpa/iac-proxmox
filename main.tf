...
module "virtual_network" {
  ...
  vn_name      = var.vm-network.name
  vn_interface = var.vm-network.interface
  vn_cidr_ipv4 = var.vm-network.cidr
  vn_vlan_id   = var.vm-network.vlan_id
  vn_bridge    = var.vm-network.bridge
...
module "virtual_machine" {
  ...
  vm_network_vlan_id          = var.vm-network.vlan_id
  vm_network_dns_server       = var.vm-network.dns_servers
  vm_network_dns_domain       = var.vm-network.dns_domain
  vm_network_internal_gateway = var.vm-network.internal_gw
  vm_network_internal_bridge  = var.vm-network.internal_bridge
